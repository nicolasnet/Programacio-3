 ...
 * @apiParam {Callable} next  The next middleware callable.
   ...
   */
	...
	public function VerificarJWTReturHora($request, $response, $next) {
	   ...
	   if($objDelaRespuesta->esValido)
	   {
		   ...
		   if($segundos%2==0){
			...
			$objDelaRespuesta->respuesta="Prohibido Pasar";
		   }		   
	...
	public function VerificarPerfilUsuario($request, $response, $next) {
	   ...
	   $objDelaRespuesta->respuesta="";
		...
		if($objDelaRespuesta->esValido)
		{
			...
			$payload=AutJWT::ObtenerData($token);
			if($payload[0]->tipo=="admin")
			{
				$response = $next($request, $response);
			}		           	
			else
			{	
				$objDelaRespuesta->respuesta=$payload[0]; 
			}		          
		}    
		else
		{
